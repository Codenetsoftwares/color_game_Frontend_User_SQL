  ...
  return (
    <>
      <div className="footerBox">
              ...
              <ul className="nav justify-content-center border-bottom pb-3 mb-3 ">
                {footerimage.map((image, index) => (
                  ...
                  <li className="nav-item col-md-1 col-sm-1 col-1">
                    <img key={index} src={image.image} alt={`footer-image-${index}`} className="imagecontainer" />
                  </li>
                  // </div>
                ))}
              </ul>
        ...
        <div className="marquee">{warning ? <marquee className="text-white fs-6">{warning}</marquee> : null}</div>
      </div>
    </>
  );
};
